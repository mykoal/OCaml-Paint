...
(* Task 6 *)
let with_thickness_slider (g: gctx) (x: int) : gctx =
  { g with thickness_slider = x }
...
let set_graphics_state (gc: gctx) : unit =
  ...
  let b = gc.thickness in 
    if b
    then Graphics.set_line_width gc.thickness_slider 
...
let draw_points (g: gctx) (pos_l: position list) : unit =
  set_graphics_state g;
  List.iter (draw_point g) (pos_l)
